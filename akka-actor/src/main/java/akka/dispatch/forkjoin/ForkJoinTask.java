 ...
 * latter.
 *
 * <p>It is possible to define and use ForkJoinTasks that may block, but doing do requires three
 ...
 * RecursiveAction} for most computations that do not return results, {@link RecursiveTask} for
 * those that do, and {@link CountedCompleter} for those in which completed actions trigger other
 ...
 * the use of {@link Phaser}, {@link #helpQuiesce}, and {@link #complete}) that may be of use in
 * constructing custom subclasses for problems that are not statically structured as DAGs. To
...
public abstract class ForkJoinTask<V> implements Future<V>, Serializable {
  ...
  /**
   * Tries to set SIGNAL status unless already completed. Used by ForkJoinPool. Other variants are
   * directly incorporated into externalAwaitDone etc.
   *
   ...
   * values.
   *
   * <p>Note: These statics are initialized below in static block.
   */
   ...
   * traces, if the exception was not thrown by the current thread, we try to create a new exception
   * of the same type as the one thrown, but with the recorded exception as its cause. If there is
   * no such constructor, we instead try to use a no-arg constructor, followed by initCause, to the
   * same effect. If none of these apply, or any fail due to other exceptions, we return the
   * recorded exception, which is still correct, although it may contain a misleading stack trace.
   *
  ...
  /**
   * Returns the result of the computation when it {@link #isDone is done}. This method differs from
   * {@link #get()} in that abnormal completion results in {@code RuntimeException} or {@code
  ...
  /**
   * Possibly executes tasks until the pool hosting the current task {@link ForkJoinPool#isQuiescent
   * is quiescent}. This method may be of use in designs in which many tasks are forked, but none
   * are explicitly joined, instead executing them until all are processed.
   */
  ...
  /**
   * Returns the pool hosting the current task execution, or null if this task is executing outside
   * of any ForkJoinPool.
   *
   * @see #inForkJoinPool
